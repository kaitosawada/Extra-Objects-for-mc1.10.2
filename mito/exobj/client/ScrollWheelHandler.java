		...
		if (event.getDwheel() != 0 && player != null) {
			...
			if (stack != null) {
				...
				if (item instanceof ItemBraceBase) {
					boolean flag = ((ItemBraceBase) item).wheelEvent(player, stack, proxy.getKey(), event.getDwheel());
					event.setCanceled(flag);
					if (flag)
						PacketHandler.INSTANCE.sendToServer(new ItemBarPacketProcessor(player.inventory.currentItem, proxy.getKey(), event.getDwheel()));
				}
			}
		}
