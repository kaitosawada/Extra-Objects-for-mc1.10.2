...
import com.mito.exobj.network.RequestChunkPacketProcessor;
...
public class LoadClientWorldHandler {
	...
	public static void onChunkUpdate(World world, BlockPos pos) {
		...
		int j = pos.getY();
		int k = pos.getZ();
		if (!ChunkAndWorldManager.existChunkData(Main.proxy.getClientWorld(), i / 16, k / 16)) {
			return;
		}
		BB_DataChunk ret = ChunkAndWorldManager.getChunkDataNew(world, i / 16, k / 16);
		if (ret != null) {
			ret.updateRenderer();
		}
	}
	...
	public void onChunkUnload(ChunkEvent.Unload e) {
		BB_DataChunk chunkData = ChunkAndWorldManager.getChunkDataNew(e.getChunk());
